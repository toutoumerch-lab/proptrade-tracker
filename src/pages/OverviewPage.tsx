...
import { mockMetrics, mockEquityCurve, mockAccounts, mockRecentWins, mockNews } from "@/data/mockData";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <MetricCard
          label="Total Equity"
          value={`$${mockMetrics.totalEquity.toLocaleString()}`}
          sub={`↑ +${mockMetrics.equityChange}% from last week`}
          icon={Wallet}
          delay={0}
        />
        <MetricCard
          label="Daily P&L"
          value={`+$${mockMetrics.dailyPnl.toLocaleString()}`}
          sub={`Open positions: ${mockMetrics.dailyPnlPositions}`}
          icon={TrendingUp}
        ...
        <MetricCard
          ...
          value={String(mockMetrics.activeAccounts).padStart(2, "0")}
          sub="08 Platforms"
          icon={Users}
          delay={160}
        />
        <MetricCard
          label="Total Risk"
          value={`$${mockMetrics.totalRisk.toLocaleString()}`}
          sub={`${mockMetrics.totalRiskPct}% of total equity`}
          icon={Shield}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className="card-surface rounded p-4 animate-fadeInUp delay-400">
          ...
          <div className="space-y-2">
            {mockAccounts.slice(0, 4).map(acct => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="card-surface rounded p-4 animate-fadeInUp delay-400">
          ...
          <div className="space-y-3">
            {mockRecentWins.map((w, i) => (
              <div key={i} className="flex items-center justify-between">
                <div>
                  <div className="text-sm font-medium text-foreground">{w.trade}</div>
                  <div className="text-2xs text-muted-foreground">{w.account}</div>
                </div>
                <div className="text-sm font-bold text-profit font-mono">+${w.pnl.toLocaleString()}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card-surface rounded p-4 animate-fadeInUp delay-500">
          ...
          <div className="flex flex-col items-center">
            <HealthGauge score={mockMetrics.globalHealthScore} />
